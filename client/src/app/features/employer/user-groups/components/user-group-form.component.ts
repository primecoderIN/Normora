    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm" (click)="onCancel()">
      <div class="w-full max-w-md bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col" (click)="$event.stopPropagation()">
        <div class="flex items-center justify-between p-5 border-b border-slate-100">
          <h3 class="text-lg font-bold text-slate-900 m-0">{{ userGroup() ? 'Edit group' : 'New user group' }}</h3>
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="flex flex-col gap-4 p-5">
          <label class="flex flex-col gap-1.5">
            <span class="text-sm font-semibold text-slate-700">Name <span class="text-red-500">*</span></span>
            <input
              type="text"
          ...
          <div class="flex items-center justify-end gap-3 mt-4 pt-4 border-t border-slate-100">
            ...
            <p-button [label]="userGroup() ? 'Save changes' : 'Create group'" type="submit" [disabled]="form.invalid || isSaving()" [icon]="isSaving() ? 'pi pi-spin pi-spinner' : ''" styleClass="!bg-primary-600 !border-primary-600 !text-white hover:!bg-primary-700 disabled:!bg-slate-300 disabled:!border-slate-300 disabled:!text-slate-500 font-bold !px-4 !py-2 !h-9 !text-sm transition-colors"></p-button>
          </div>
        </form>
      </div>
    </div>
