      ...
      <div class="w-full max-w-md bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col" (click)="$event.stopPropagation()">
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="flex flex-col gap-4 p-5">
          ...
          <div class="flex items-center justify-end gap-3 mt-4 pt-4 border-t border-slate-100">
            ...
            <p-button [label]="userGroup() ? 'Save changes' : 'Create group'" type="submit" [disabled]="form.invalid || isSaving()" [icon]="isSaving() ? 'pi pi-spin pi-spinner' : ''" styleClass="!bg-indigo-600 !border-indigo-600 !text-white hover:!bg-indigo-700 disabled:!bg-slate-300 disabled:!border-slate-300 disabled:!text-slate-500 font-bold !px-4 !py-2 !h-9 !text-sm transition-colors"></p-button>
          </div>
        </form>
      </div>
    </div>
