      ...
      <table class="w-full text-left text-sm whitespace-nowrap">
        <thead class="bg-slate-50/50 border-b border-slate-200">
          <tr>
            ...
            <th class="px-5 py-3 font-bold text-slate-700">Members</th>
            <th class="px-5 py-3 font-bold text-slate-700">Departments</th>
            <th class="px-5 py-3 font-bold text-slate-700">Created</th>
            <th class="px-5 py-3 font-bold text-slate-700 w-32"></th>
          </tr>
        </thead>
        <tbody class="divide-y divide-slate-100">
          @for (group of groups(); track group.id) {
            <tr class="hover:bg-slate-50/50 transition-colors group/row">
              ...
              <td class="px-5 py-3.5 text-slate-500 truncate max-w-xs">{{ group.description || '—' }}</td>
              <td class="px-5 py-3.5">
              ...
              <td class="px-5 py-3.5 text-slate-500">{{ group.createdAt | date:'mediumDate' }}</td>
              <td class="px-5 py-3.5">
                <div class="flex items-center justify-end gap-1 sm:opacity-0 sm:group-hover/row:opacity-100 focus-within:opacity-100 transition-opacity">
                  <p-button icon="pi pi-sliders-h" (onClick)="manage.emit(group)" title="Manage assignments" styleClass="!w-8 !h-8 !p-0 flex items-center justify-center !bg-transparent !border-transparent !text-slate-400 hover:!bg-slate-200 hover:!text-slate-700 transition-colors rounded-lg"></p-button>
                  <p-button icon="pi pi-pencil" (onClick)="edit.emit(group)" title="Edit" styleClass="!w-8 !h-8 !p-0 flex items-center justify-center !bg-transparent !border-transparent !text-slate-400 hover:!bg-slate-200 hover:!text-slate-700 transition-colors rounded-lg"></p-button>
                  <p-button icon="pi pi-trash" (onClick)="delete.emit(group)" title="Delete" styleClass="!w-8 !h-8 !p-0 flex items-center justify-center !bg-transparent !border-transparent !text-slate-400 hover:!bg-red-50 hover:!text-red-600 transition-colors rounded-lg"></p-button>
                </div>
              </td>
            </tr>
          }
